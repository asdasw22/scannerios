...
struct AnswerKeyView: View {
    ...
    private func syncKey() { var key = exam.answerKey ?? AnswerKey(name: "\(exam.name) Key"); if exam.answerKey == nil { context.insert(key) }; key.entries = Dictionary(uniqueKeysWithValues: exam.questions.compactMap { $0.correctAnswer.map { ($0.number, $0) } }); exam.answerKey = key; try? context.save() }
}